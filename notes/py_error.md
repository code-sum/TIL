...
- [VS Code] breakpoint 활용한 디버깅
  ...
  1. `launch.json` 파일 만들기
     ...
     - `launch.json 파일 만들기` 누르면 오른쪽 이미지처럼 `디버그 구성 선택` 창이 뜸
  ...
  3. 디버그 실행 및 도구모음 버튼 의미

     - 왼쪽 메뉴에서 디버그 창을 선택, 위쪽 초록색 버튼을 누르면 디버그 실행
     - 왼쪽 창에서 현재 변수의 값 확인 가능
     - 중앙 위에 나오는 버튼이 도구모음. 각 버튼은 아래의 의미 담고있음
       1. `continue` : 다음 중단점까지 실행
       2. `step over` : 한 줄 실행. 함수가 있으면 실행 후 넘어감(내부로 들어가지 않음)
       3. `step into` : 한 줄 실행. 함수 내부로 들어감
       4. `step out` : 한 줄 실행. 함수 내부에 있다면 함수를 끝까지 실행시키고 호출시킨 곳으로 돌아감
       5. `restart` : 디버그 다시 시작
       6. `stop` : 디버그 중지

  - [파일 읽기를 통한 입력 처리 & 디버그 (notion.so)](https://www.notion.so/hphk-edu/cffd699a97f045d98da99e37b3e1d905)
